...
class Analysis::QuestionSynthesisTest < ActiveSupport::TestCase
  test "perform for spanish language" do
    VCR.use_cassette("question_synthesis/es") do
      ...
      assert_equal 5, analysis.reload.questions.count
      assert_equal "¿Cuáles son los coches más seguros recomendados para mujeres mayores de 45 años?", analysis.questions.first["question"]
    end
  end
  ...
  test "perform for german language" do
    VCR.use_cassette("question_synthesis/deu") do
      ...
      assert analysis.perform_and_save
      assert_equal 5, analysis.reload.questions.count
      assert_equal "Welche Automarken bieten die sichersten Fahrzeuge für Frauen über 45 Jahren an?", analysis.questions.first["question"]
    end
  end
  ...
  test "perform for french language" do
    VCR.use_cassette("question_synthesis/fra") do
      ...
      assert analysis.perform_and_save
      assert_equal 5, analysis.reload.questions.count
      assert_equal "Quelles sont les voitures les plus sûres recommandées pour les femmes de plus de 45 ans ?", analysis.questions.first["question"]
    end
  end
  ...
  test "perform for english language" do
    VCR.use_cassette("question_synthesis/en") do
      ...
      assert analysis.perform_and_save
      assert_equal 5, analysis.reload.questions.count
      assert_equal "What are the safest car models recommended for women over 45?", analysis.questions.first["question"]
    end
  end
